...
export default (props) => {
	...
	if (props.this.state.tableData.length === 0){
		...
		tData = chunkedData[props.this.state.activePage-1].map((colName) => {
			console.log(colName.rDurations.length)
			var durations = colName.rDurations.map((duration) => {
				j++;
				var hour = Math.floor(duration/ (1000*60*60));
				var showHour = (hour < 10) ? "0"+hour : hour;
				var minute = Math.floor((duration/60000) % 60);
				var showMinute = (minute < 10) ? "0"+minute : minute;
				var second = Math.floor((duration/1000) % 60);
				var showSecond = (second < 10) ? "0"+second : second;
				var showDura = (duration == "-") 
					? duration
					: ""+showHour+":"+showMinute+":"+showSecond;
				if (duration == "-" || (minute < 3 || (minute == 3 && second == 0))) {
					return (<td key={j} style={{textAlign:"right", paddingRight:"15px"}}>{showDura}</td>);
				}else {
					return (<td key={j} style={{textAlign:"right", paddingRight:"15px" , color:"red"}}>{showDura}</td>);
				}
			});
			i++;
			return(
